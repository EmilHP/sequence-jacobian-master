...
def household(Vd_p, Vb_p, Pi_e_p, b_grid, dg_grid, k_grid, e_grid, e_ergodic, sigma_N, sigma_D,
              alpha,delta,r,kappa,Div,beta,Ne,Nb,Nd,Nk,P,P_d,P_n,W,N,tau_e,Tax):
...
def solve_unconstrained(sigma_N,alpha,delta,r_grid,d_grid,b_grid,
    ...
    # a. EGM loop find d' conditional on b' choice
    ...
    for ie in prange(Ne): # parallel
        for ib in range(Nb):
            for i_d in range(Nd):
                ...
                if c[ie, ib, i_d] <= 0:
                    c[ie, ib, i_d] = 1e-6 # for numerical stability
                ...
                b_endo_unc[ie, ib, i_d] = (P_n_p*c[ie, ib, i_d] + P_d_p*(dp_endo[ie, ib, i_d] - (1 - delta) * d_grid[i_d]) + b_grid[ib] + Psi_fun(alpha,delta,dp_endo[ie, ib, i_d],d_grid[i_d]) -
                            z_grid[ie] - lump[ie,ib]) / (1 + r_grid[ib])
    
    return dp_endo, Wb_endo, b_endo_unc, c

@njit(fastmath=True,parallel=True)
def solve_constrained(sigma_N,alpha,delta,r_grid,d_grid,b_grid,k_grid,
    ...
    for ie in prange(Ne): # parallel
        for ik in range(Nk):
            for i_d in range(Nd):
                ...
                if c[ie, ik, i_d] <= 0:
                    c[ie, ik, i_d] = 1e-6 # for numerical stability
                ...
                b_endo_con[ie, ik, i_d] = (P_n_p*c[ie, ik, i_d] + P_d_p*(dp_endo[ie, ik, i_d] - (1 - delta) * d_grid[i_d]) + b_grid[0] + Psi_fun(alpha,delta,dp_endo[ie, ik, i_d],d_grid[i_d]) -
                            z_grid[ie] - lump[ie,0]) / (1 + r_grid[0])
    
    return dp_endo, Wb_endo, b_endo_con, c

def collect_policy(delta,alpha,r_grid,Ne,Nb,Nd,b_grid,zzz,lll,ddd,bbb,d_unc,b_unc,d_con,P_n_p,P_d_p):
    ...
    c[:] = (zzz + lll - P_d_p*(d - (1 - delta) * ddd ) + (1 + r_grid[np.newaxis, :, np.newaxis]) * bbb - Psi_fun(alpha,delta, d, ddd)  - b)/P_n_p
    
    return d, b, c

def time_iteration(sigma_N,sigma_D,alpha,delta,r_grid,lump,beta,Pi_e,d_grid,z_grid,b_grid,k_grid,zzz,lll,bbb,ddd,
    ...
    # a. pre-compute post-decision value functions and LHS of first-order optimality equation
    ...
    dp_endo_unc, _, b_endo_unc, _ = solve_unconstrained(sigma_N,alpha,delta,r_grid,d_grid,b_grid,
                        z_grid,lump,Ne,Nb,Nd,lhs_unc,rhs,Wb,P_d_p,P_n_p)
    ...
    dp_endo_con, _, b_endo_con, _ = solve_constrained(sigma_N,alpha,delta,r_grid,d_grid,b_grid,k_grid,
                      z_grid,lump,Ne,Nk,Nd,lhs_con,rhs,Wb,P_n_p,P_d_p)
    ...
    d, b, c = collect_policy(delta,alpha,r_grid,Ne,Nb,Nd,b_grid,zzz,lll,ddd,bbb,d_unc,b_unc,d_con,P_n_p,P_d_p)
    ...
    uc = c**(-1/sigma_N)
    ud = (ddd)**(-1/sigma_D)
    rrr = r_grid[np.newaxis, :, np.newaxis]
