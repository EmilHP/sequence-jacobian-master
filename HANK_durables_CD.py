...
def household(Vd_p, Vb_p, Pi_e_p, b_grid, dg_grid, k_grid, e_grid, e_ergodic, sigma_N, sigma_D,
    ...
    theta = 0.507 # share of non-durable consumption in CD utility
    ...
    # c. compute time step 
    Vb,Vd,dg,b,c,c_d = time_iteration(sigma,theta,alpha,delta,r,lump,beta,Pi_e_p,dg_grid,z_grid,b_grid,k_grid,zzz,lll,bbb,ddd,
...
def solve_unconstrained(sigma,theta,alpha,delta,r,d_grid,b_grid,
...
@njit(fastmath=True,parallel=True)
def solve_constrained(sigma,theta,alpha,delta,r,d_grid,b_grid,k_grid,
...
def time_iteration(sigma,theta,alpha,delta,r,lump,beta,Pi_e,d_grid,z_grid,b_grid,k_grid,zzz,lll,bbb,ddd,
    ...
    dp_endo_unc, _, b_endo_unc, _ = solve_unconstrained(sigma,theta,alpha,delta,r,d_grid,b_grid,
                        z_grid,lump,Ne,Nb,Nd,lhs_unc,rhs,Wb,P_d_p,P_n_p)
    ...
    dp_endo_con, _, b_endo_con, _ = solve_constrained(sigma,theta,alpha,delta,r,d_grid,b_grid,k_grid,
                      z_grid,lump,Ne,Nk,Nd,lhs_con,rhs,Wb,P_n_p,P_d_p)
    ...
    uc = theta*c**(theta-1)*(ddd)**(1-theta)*((c**theta*(ddd)**(1-theta)))**(-sigma)
    ud = (1-theta)*c**theta*(ddd)**(-theta)*((c**theta*(ddd)**(1-theta)))**(-sigma)
